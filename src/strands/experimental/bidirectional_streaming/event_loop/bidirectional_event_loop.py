...
async def start_bidirectional_connection(
...
async def _handle_interruption(session: BidirectionalConnection) -> None:
    ...
    async with session.interruption_lock:
        ...
        logger.debug(
            "Interruption handled - tools cancelled: %d, audio cleared: %d",
            cancelled_tools,
            cleared_count,
        )
...
async def _execute_tool_with_strands(session: BidirectionalConnection, tool_use: dict) -> None:
    ...
    try:
        ...
        # Import structured output context
        from ....tools.structured_output._structured_output_context import (
            StructuredOutputContext,
        )

        # Bidirectional agents don't use structured output (streaming mode)
